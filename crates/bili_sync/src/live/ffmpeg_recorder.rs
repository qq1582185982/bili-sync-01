...
impl FFmpegRecorder {
    ...
    pub async fn start_with_cdn(&mut self, stream_url: &str, cdn_node: &str) -> Result<()> {
        live_info!("FFmpeg录制器开始录制流: {}, CDN: {}", stream_url, cdn_node);
    ...
    async fn start_recording(&mut self, stream_url: &str, cdn_node: &str) -> Result<()> {
                    ...
                    if line == "progress=continue" {
                        ...
                        if let Some(speed) = progress_data.get("speed") {
                            if let Some(speed_str) = speed.strip_suffix('x') {
                                if let Ok(speed_val) = speed_str.parse::<f64>() {
                                    if speed_val < 0.95 {
                                        live_warn!("录制速度过慢: {}x，可能丢帧", speed_val);
                                    } else if speed_val > 1.05 {
                                        live_debug!("录制速度: {}x", speed_val);
                                    }
                                }
                            }
                        }
                        ...
                        if let Some(fps) = progress_data.get("fps") {
                            if let Ok(fps_val) = fps.parse::<f64>() {
                                if fps_val < 20.0 {
                                    live_warn!("当前FPS过低: {}", fps_val);
                                } else {
                                    live_debug!("当前FPS: {}", fps_val);
                                }
                            }
                        }
                    ...
                    if !line.is_empty() {
                        ...
                        if line.contains("403 Forbidden") || line.contains("404 Not Found") || 
                           line.contains("HTTP error 403") || line.contains("HTTP error 404") {
                            live_error!("检测到URL过期错误，FFmpeg将快速失败: {}", line);
                        } else if line.contains("error") || line.contains("Error") || line.contains("ERROR") {
                            live_error!("FFmpeg错误: {}", line);
                        } else if line.contains("warning") || line.contains("Warning") || line.contains("WARNING") {
                            live_warn!("FFmpeg警告: {}", line);
                        } else {
                            live_debug!("FFmpeg输出: {}", line);
                        }
                    }
                }
                
                live_debug!("FFmpeg进程 {:?} 的stderr读取已结束", process_id);
            });
        }
        ...
        live_info!("FFmpeg录制已启动，PID: {:?}", self.primary_process.as_ref().map(|p| p.id()));
        Ok(())
    }

    /// 停止录制
    pub async fn stop(&mut self) -> Result<()> {
        live_info!("停止FFmpeg录制");
        ...
        if let Some(mut process) = self.primary_process.take() {
            ...
            if let Some(mut stdin) = self.primary_stdin.take() {
                match stdin.write_all(b"q").await {
                    Ok(_) => {
                        if let Err(e) = stdin.flush().await {
                            live_warn!("flush stdin失败: {}", e);
                        }
                        live_info!("已发送优雅停止命令到FFmpeg进程 {:?}", pid);
                        ...
                        loop {
                            match process.try_wait() {
                                Ok(Some(status)) => {
                                    live_info!("FFmpeg优雅退出，PID: {:?}，状态: {:?}", pid, status);
                                    break;
                                }
                                Ok(None) => {
                                    if start.elapsed() > timeout {
                                        live_warn!("FFmpeg未在{}秒内响应停止命令，强制终止", timeout.as_secs());
                                        if let Err(e) = process.kill().await {
                                            live_error!("强制终止进程失败: {}", e);
                                        }
                                        break;
                                    }
                ...
                live_warn!("stdin不可用，强制终止FFmpeg进程 {:?}", pid);
                if let Err(e) = process.kill().await {
                    live_error!("强制终止进程失败: {}", e);
                }
            }
            ...
            if let Err(e) = process.wait().await {
                live_warn!("等待FFmpeg进程退出失败: {}", e);
            }
        }

        live_info!("FFmpeg录制已停止");
        Ok(())
    }
    ...
    pub fn check_process_status(&mut self) -> Result<bool> {
        if let Some(ref mut process) = self.primary_process {
            match process.try_wait() {
                Ok(Some(status)) => {
                    if status.success() {
                        live_info!("FFmpeg进程正常退出");
                    } else {
                        live_error!("FFmpeg进程异常退出: {:?}", status);
                    }
                    Ok(false)
                }
                Ok(None) => Ok(true), // 仍在运行
                Err(e) => {
                    live_error!("检查进程状态失败: {}", e);
