...
- **老视频在线播放支持拖动（seek）**
  - 在线播放遇到 FLV 混合流时，默认改为原始代理（保留 Range），前端使用 `flv.js`（MSE）实现可拖动播放
  - `proxy-stream` 新增 `transmux=1` 兼容开关：强制服务端转封装 mp4（fragmented mp4），用于不支持 `flv.js` 的环境
- **修复老视频混合流无法触发 FLV 播放逻辑**
  - 在线播放返回混合流时增加容器类型标识（FLV/MP4），前端据此选择 flv.js 或原生播放路径
- **修复 flv.js 初始化只触发一次导致不发起视频流请求**
  - 监听播放状态、流信息与 video 元素变更，确保在线模式下能正确创建 flv.js 播放实例
- **提升 flv.js 兼容性与容错**
...
- **当前版本按钮增加 beta 镜像更新角标提示**
  ...
  - 点击版本号显示详情悬浮窗（移动端无悬停也可查看），不再跳转到更新记录页面
- **前端时间显示统一为北京时间**
  - 首页监控/任务、任务队列、投稿日期与图表 tooltip 统一使用 `Asia/Shanghai`（24 小时制），避免因浏览器时区导致显示不一致
  - 顶栏“当前版本”悬浮提示中的镜像时间改为本地统一格式（去掉 RFC3339 的 `T`/纳秒），与页面其它时间风格一致
...
- **修复部分视频在低画质配置下 `no video stream found`**
  ...
  - 在线播放接口补充使用分页 `duration` 生成弹幕防挡参数，避免 `duration=0` 造成请求参数异常
- **修复番剧分页 CID 异常导致弹幕/字幕失败**
  - 番剧季缓存里遇到无效 CID（如 `-1`）时不再当作有效缓存使用
  - 下载阶段发现分页 CID 无效时，会尝试用 `video.cid` 或番剧 API 进行自愈修复后再继续
- **修复平铺目录下删除视频源误删同目录文件**
  - 当 `flat_folder=true` 且多个视频源共用同一目录时，删除单个视频源并选择“删除本地文件”不再递归删除源目录
  - 改为仅删除“解绑后变成孤立”的视频所对应的 `page.path` 文件与侧车文件（避免影响仍被其他视频源引用的视频）
- **平铺目录模式下禁用 TVShow/Season 元数据**
  - 番剧/合集/多P在 `flat_folder=true` 时不再创建 `Season 01` 等目录结构
  - 同时跳过下载 `tvshow.nfo`、`poster.jpg`、`folder.jpg`、`Season01-*.jpg`、`*-thumb.jpg`、`*-fanart.jpg`
- **删除视频源确认文案空格显示修复**
  - 删除确认弹窗中的视频源名称改为保留连续空格显示，避免复制后与实际名称不一致导致无法确认删除
- **修复老视频仅返回 FLV 混合流导致网页端无法播放**
  - 下载阶段遇到 FLV 混合流时，先下载后使用 `ffmpeg -c copy` 转封装为 mp4（`+faststart`）
  - 在线播放 `proxy-stream` 遇到 FLV 时自动转封装输出 mp4（fragmented mp4），兼容浏览器播放
  - 本地播放接口检测到“FLV 内容被保存为 .mp4”时，会自动转封装并原地替换为可播放文件
